...
def send_request(host, port, request):
    ...
    #Create a new socket in with block to ensure its closed once we're done
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client_socket:
        # Connect the socket ot host:port
        client_socket.connect((host,port))
        # Send the request through the connected socket.
        client_socket.send(request)
        # Shut the socket to further writes. Tell server we're done sending
...
def handle_connection (conn, addr):
    with conn:
        ...
        while True: #While the client is keeping the socket open
            data = conn.recv(BYTES_TO_READ)#read some data from the sacket
...
def start_server():
    '''
    Create the socket in 'with' block to ensure it gets autoclosed once we're done
    '''
    ...
    # Bind the server to a specific host and port on this machine
        ...
        '''
        Wait for an incomming connection, and when one arrives, accept it and create a new socket called conn' to interact with it.
        '''
        conn, addr = server_socket.accept()
        #Pass 'conn' off to handle connection to do some logic handle_connection (conn, addr)
